...
| MA2 | 备用源接入 ≥2 个 | ✅ **2026-09-21 完成（MA2a + MA2b 两个备用源均已进链）**。Google Patents XHR 直抓（免费无 Key，规格书§2，含限速退避）**✅ MA2a** + EPO OPS（规格书§4，免费注册英文/EU 域补源，排链尾）**✅ MA2b**；在线链现为 `[SerpAPI(有 Key) → GooglePatentsXhr → EpoOps(有 Key)]`。**验收口径拆分**：「人为禁用 SerpAPI 后自动降级仍出结果」已由链级用例离线取证（`search::chain` 的 case1/case4/case5，注入假传输零联网）；「诊断面板标红失败源」的**前端可见性属 MA5**（本里程碑只保证 `attempts` 全量记账且 error 点名来源） | MA1 |
| MA2a | Google Patents XHR 直抓源 | ✅ **2026-09-20 完成**（分支 `exec/ma2a-xhr`，提交链 `0ac73e7`(源+SourceChain) → `4bca304`(测试) → `52948ad`(routes 接线)；起点 `0c37569` 接手第五棒契约半成品）：落地 `src/search/chain.rs::SourceChain`（spec §6 并行发起 + 按登记顺序择胜）与 `src/search/providers/google_patents_xhr.rs`（spec §2 唯一 query 端点、参数集、反爬 503→`FailKind::Quota`+指数退避并尊重 `Retry-After`、客户端日期窗口、`page>1` 诚实 hint）；`/api/search/online` 接上降级链（SerpAPI 主 → XHR 免费降级），`source` 字段自 MA2a 起如实反映胜出源，其余出参形状与本地兜底逐字未变。**证据**：降级链 spec §6 三用例（SerpAPI 挂→降级非空 / XHR 限频→退避重试→成功 / 双挂→`attempts` 各带 `FailKind`）+ XHR 源 24 条离线实证单测，**全部注入假传输与假时钟，零联网**；上游超时按 spec §1 收紧为 **15s**（SerpAPI 维持旧 30s 未动）。**门禁（收尾 agent 第七棒，tip `52948ad`，rustc/clippy 1.98.0）**：`cargo fmt --check` exit 0 ｜ `cargo clippy --all-targets -- -D warnings` exit 0（改动文件已 touch 强制复检）｜ `cargo test` **525 passed / 0 failed / 3 ignored**（main 基线 457 → 525，净增 68 = 新增 34 条 `#[test]`/`#[tokio::test]` 在 lib 与 bin 两个测试二进制各计一次）｜ `src/common.rs` **零 diff**（71 条 `.route()` 未动）｜ templates/ 与 static/ **零 diff** ⇒ HTML 函数扫描与 Puppeteer e2e 按 DoD 条件不适用；`Cargo.toml`/`Cargo.lock` 零 diff（无新依赖）、`src/db/` 零 diff（无 schema 变更）；生产路径零 `unwrap`/`expect`（全部命中点均在 `#[cfg(test)]` 内）。**已开 PR #12 待审**，交接与验收数字见 MASTER §5 第十二次交接 | MA1 |
| MA2b | EPO OPS 第三源 | ✅ **2026-09-21 完成**（分支 `exec/ma2b-epools`，提交链 `f800f18`(provider 主体，第八棒) → `1c6d6f0`(routes 接线) → `edff7c7`(链级用例) → `0bd7921`(dead_code 对账)）：落地 `src/search/providers/epo_ops.rs`（规格书§4：OAuth2 client_credentials 取 token + 进程内缓存与到期前 60s 提前刷新 + 撞 401/403 作废重取一次并重试；检索走 `published-data/search/biblio`——规格书原写的无 constituent 端点响应**只有公开号无标题**，会被共用的「空标题丢弃」恒定清零，故按官方 Reference Guide v1.3.20 更正，取证见模块头 EVIDENCE L4/L6；CQL 索引目录里 `cn` 是 IPC 附加分类不是国家，故国家限定用裸码；15s 超时、传输与时钟可注入）。`/api/search/online` 的链登记为 `[SerpAPI(有 Key) → GooglePatentsXhr → EpoOps(有 Key)]`，凭证读 `app_settings` 的 `EPO_KEY`/`EPO_SECRET`（环境变量后备，与 SerpAPI 同一条 `get()` 路径），**缺任一半即不登记该源**（`attempts` 里不会出现它，与 SerpAPI 缺 Key 同语义）；`source` 字段自本棒起可多出一个取值 `"epo_ops"`，其余出参形状与本地兜底逐字未变。**定位**：英文/EU 域补充源，排链尾——OPS 的 `ti`/`ab` 只有英文著录索引，中文零命中是其真实能力边界，代码与用例均不伪装成成功。**证据**：provider 侧 39 条离线用例（第八棒）+ 链级 3 条（case4 前两源挂→EPO 顶上且 `winning_source` 如实 / case5 三源全挂→空结果但 `attempts` 三条各带 Auth·Quota·Auth 且 error 点名来源 / case6 无凭证登记态→Skipped 且零请求）+ routes 侧链形状 1 条 + 配置读取 2 条，**全程注入假传输与假时钟，零联网**。**门禁（第九棒，tip `0bd7921`，rustc/clippy 1.98.0）**：`cargo fmt --check` exit 0 ｜ `cargo clippy --all-targets -- -D warnings` exit 0（9 个改动文件 touch 后强制复检）｜ `cargo test` **615 passed / 0 failed / 3 ignored**（main 基线 525 → 615，净增 90 = 新增 45 条 `#[test]`/`#[tokio::test]`（epo_ops 39 + chain 3 + routes 3）在 lib(237→282) 与 bin(239→284) 两个测试二进制各计一次）｜ `src/common.rs` 零 diff（**118** 条 `.route()` 与 main 逐条全等）｜ templates/ 与 static/ 零 diff ⇒ HTML 扫描与 Puppeteer e2e 按 DoD 条件不适用 ｜ `Cargo.toml`/`Cargo.lock` 零 diff（无新依赖）｜ `src/db/` 零 diff ｜ 生产路径零 `unwrap`/`expect`。**遗留**：① 真实凭证下的 `search/biblio` 冒烟（核对 `total-result-count` 与首条标题）与 XHR 的降级出参冒烟同批做，属 MA5；② 设置页 `EPO_KEY`/`EPO_SECRET` 两个配置位属 MA5（读取侧已就绪，落库即生效）；③ `MergedPatent.key`→MA4、`FailKind::cools_down()`→MA5/MA6、`SourceKind::LocalFts`→MA4 仍无生产消费点，标注如实保留未造假。**开 PR 待审**，交接与验收数字见 MASTER §5 第十三次交接 | MA2a |
